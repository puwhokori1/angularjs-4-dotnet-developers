...
app.factory('catalog', function(pageMethods, $rootScope) {
	var result = [];
	pageMethods.GetCatalog(function(data) {
	
		data.forEach(function(item) {
			result.push({ id: item.Id, name: item.Name, credits: item.Credits, instructorName: item.InstructorName, registered: item.Registered });
		});
		$rootScope.$apply();
	});
