...
app.factory('catalog', function($http) {
	...
	return {
		get: function() {
			$http.get('/Data/GetCatalog').then(function(response) {
				catalog.length = 0;
				
				response.data.forEach(function(item) {
					catalog.push({id: item.Id, name: item.Name, credits: item.Credits,
						instructorName: item.InstructorName, registered: item.Registered});
				});
			});
			return catalog;
		}
